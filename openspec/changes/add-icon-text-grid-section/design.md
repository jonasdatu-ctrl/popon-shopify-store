...
### Cap of 4 via native `limit` on the block schema entry
Shopify's section schema supports a `"limit"` property on a block-type entry in the section's `"blocks"` array (already used theme-wide in the legacy `customcode-*` sections' locally-defined blocks, e.g. `customcode-step-block.liquid`'s `"limit": 4`). Declaring `{ "type": "icon-text-card", "limit": 4 }` in `icon-text-grid.liquid`'s schema achieves the "max 4" requirement while keeping `icon-text-card` a normal, reorderable, independently addable/removable block — unlike `icon-list`'s fixed-slot-settings approach, which the proposal for this change does not call for.
